    ...
    for i, cnt in enumerate(contours):
        ...
        if approx.shape[0] == 4 and area > cond_area and cv2.isContourConvex(approx) :
            ...
            if maxCosine < 0.3 :
                ...
                print(rcnt[1])
                cv2.polylines(image, [rcnt], True, (0,0,255), thickness=15, lineType=cv2.LINE_8)

                # 円を描画する
                for i in range(len(rcnt)):
                    cv2.circle(image,rcnt[i],10,(0,255,0),thickness=30,lineType=cv2.LINE_8)
    return image
